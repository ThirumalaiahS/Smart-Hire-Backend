using Microsoft.AspNetCore.Identity;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Configuration;
using Moq;
using Velora.Core.Entities;
using Velora.Infrastructure.Data;
using Velora.Infrastructure.Services;
using System.IdentityModel.Tokens.Jwt;
using Xunit;

namespace Velora.Tests.Auth
{
    public class TokenServiceTests
    {
        private readonly Mock<IConfiguration> _mockConfig;
        private readonly Mock<UserManager<AppUser>> _mockUserManager;
        private readonly AppDbContext _dbContext;
        private readonly TokenService _tokenService;

        public TokenServiceTests()
        {
            _mockConfig = new Mock<IConfiguration>();
            _mockUserManager = MockUserManager<AppUser>();

            // Setup config
            _mockConfig.Setup(x => x["JwtSettings:SecretKey"]).Returns("v9y$B&E)H@McQfTjWnZr4u7x!A%C*F-JaNdRgUkXp2s5v8y/B?E(G+KbPeShVmYq");
            _mockConfig.Setup(x => x["JwtSettings:Issuer"]).Returns("VeloraAPI");
            _mockConfig.Setup(x => x["JwtSettings:Audience"]).Returns("VeloraClient");
            _mockConfig.Setup(x => x["JwtSettings:ExpiryMinutes"]).Returns("1440");

            var options = new DbContextOptionsBuilder<AppDbContext>()
                .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
                .Options;
            _dbContext = new AppDbContext(options);

            _tokenService = new TokenService(_mockConfig.Object, _mockUserManager.Object, _dbContext);
        }

        private static Mock<UserManager<TUser>> MockUserManager<TUser>() where TUser : class
        {
            var store = new Mock<IUserStore<TUser>>();
            return new Mock<UserManager<TUser>>(store.Object, null!, null!, null!, null!, null!, null!, null!, null!);
        }

        [Fact]
        public async Task CreateToken_ReturnsTokenWithRoles()
        {
            // Arrange
            var user = new AppUser
            {
                Id = "user-id",
                Email = "test@example.com",
                FullName = "Test User"
            };
            var roles = new List<string> { "Admin", "Recruiter" };

            _mockUserManager.Setup(x => x.GetRolesAsync(user))
                .ReturnsAsync(roles);

            // Act
            var token = await _tokenService.CreateToken(user, CancellationToken.None);

            // Assert
            Assert.NotNull(token);
            var handler = new JwtSecurityTokenHandler();
            var jwtToken = handler.ReadJwtToken(token);

            Assert.Equal("test@example.com", jwtToken.Claims.First(c => c.Type == JwtRegisteredClaimNames.Email).Value);
            var roleClaims = jwtToken.Claims.Where(c => c.Type == "role").Select(c => c.Value).ToList();
            Assert.Contains("Admin", roleClaims);
            Assert.Contains("Recruiter", roleClaims);
        }

        [Fact]
        public async Task GenerateTokens_CreatesAndStoresRefreshToken()
        {
            // Arrange
            var user = new AppUser
            {
                Id = "user-id-gt",
                Email = "test@example.com",
                FullName = "Test User"
            };
            var roles = new List<string> { "Candidate" };
            _mockUserManager.Setup(x => x.GetRolesAsync(user)).ReturnsAsync(roles);

            var systemUser = new SystemUser
            {
                UserId = 10,
                IdentityUserId = "user-id-gt",
                FullName = "Test User",
                Email = "test@example.com"
            };
            await _dbContext.SystemUsers.AddAsync(systemUser);
            await _dbContext.SaveChangesAsync();

            // Act
            var result = await _tokenService.GenerateTokens(user, "127.0.0.1", CancellationToken.None);

            // Assert
            Assert.NotNull(result.Token);
            Assert.NotNull(result.RefreshToken);
            var storedToken = await _dbContext.RefreshTokens.FirstOrDefaultAsync(r => r.Token == result.RefreshToken);
            Assert.NotNull(storedToken);
            Assert.Equal(systemUser.UserId, storedToken.SysUserId);
        }

        [Fact]
        public async Task RevokeToken_RevokesTokenSuccessfully()
        {
            // Arrange
            var systemUser = new SystemUser
            {
                UserId = 11,
                IdentityUserId = "user-id-rt",
                FullName = "Test User",
                Email = "test@example.com"
            };
            await _dbContext.SystemUsers.AddAsync(systemUser);

            var refreshToken = new RefreshToken
            {
                SysUserId = 11,
                Token = "some-refresh-token",
                JwtId = "jwt-id",
                ExpiresOn = DateTime.UtcNow.AddDays(1),
                IsRevoked = false,
                IsUsed = false
            };
            await _dbContext.RefreshTokens.AddAsync(refreshToken);
            await _dbContext.SaveChangesAsync();

            // Act
            var result = await _tokenService.RevokeToken("some-refresh-token", "127.0.0.1", CancellationToken.None);

            // Assert
            Assert.True(result);
            var storedToken = await _dbContext.RefreshTokens.FirstAsync(r => r.Token == "some-refresh-token");
            Assert.True(storedToken.IsRevoked);
            Assert.NotNull(storedToken.RevokedOn);
        }
    }
}

